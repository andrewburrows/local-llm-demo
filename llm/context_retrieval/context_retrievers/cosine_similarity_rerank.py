...
class CosineSimilarityRerank(ContextRetriever):
    def __init__(self, retriever: VectorStoreRetriever, multi_query_request: str):
        ...
        self.embeddings_function = SentenceTransformerEmbeddings(model_name="all-MiniLM-L6-v2")
    ...
    def query_context_chain(self):
        ...
        def generate_responses(queries: list[str]) -> list[(Document, float)]:
            documents = []
            question_embedding = self.embeddings_function.embed_query(question)  # Embed the original question
            for query in queries:
                ...
                for doc, score in docs_with_scores:
                    doc_embedding = self.embeddings_function.embed_query(doc.page_content)  # Embed the document content
    ...
    def _reciprocal_rank_fusion(self, retrieved_documents: list[(Document, float)]):
        sorted_documents = sorted(retrieved_documents, key=lambda document: document[1], reverse=True)
        print(f"-----| Documents retrieved |-----")
        for document, score in retrieved_documents:
            print(f"{score}: {document.page_content}")
